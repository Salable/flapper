...
async function fireDueInterrupters({ broker, db, board, type }) {
  ...
  await boards.updateInterrupters(db, board.id, (existing) => {
    toFire = dueInterrupters(existing, nowMs, { createdAtMs: board.createdAt?.getTime?.() ?? 0 });
  ...
  for (const { preset } of [...toFire].reverse()) {
    try {
      await admit(db, board, type, interrupterBody(preset), 'schedule');
    } catch (error) {
 ...
 */
function interrupterBody(preset) {
