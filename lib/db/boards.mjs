...
/**
 * One board, the same shape `listPublic` gives the gallery - joined to its
 * *current* queue item (left-joined; an idle board has none) rather than
 * just its row. For anything that needs to show what a board is actually
 * saying rather than just its name (the og-image route, `/b/[slug]`'s own
 * metadata) - `getBySlug` alone has no `currentPayload` to give it.
 */
export async function getBySlugWithCurrent(db, slug) {
  const [row] = await db
    .select({
      id: boards.id,
      slug: boards.slug,
