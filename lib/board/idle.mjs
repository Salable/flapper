...
/**
 * A fidget style: the whole shape of "what kind", as against `ambientMs`'s
 * "how often". Three groups of field - how often it does anything, how far
 * and how fast the card goes, and what colour it wears on the way - plus
 * `traveller`, which replaces all of them with a creature.
 *
 * Not exported: it is what FIDGET_STYLES is built from and what idleAction
 * merges over, both in this file. A style reaches a caller as a
...
const FIDGET_DEFAULTS = Object.freeze({
  ...
  stepMs: null,
  /**
   * Travel speed for the way home only, in ms per step; null falls back to
   * `stepMs`, then to the board's own. Two legs, two speeds: a tick hurries
   * back so a small gesture stays small, a drink comes back as slowly as it
   * went.
   */
  returnStepMs: null,
  /**
   * Colours to bake whole cards in while they move, or null.
   *
   * This is the only way a fidget wears colour. `flight` - the pack property
   * that lays a fill over the finished tile every frame - is deliberately
   * not available to one, and Dan's reason is the whole argument: **a colour
   * overlay only works on monochrome cards.** A tint asks the card's own
   * paint what the answer should be, so the same pineapple comes back grey
   * on a dark design, muddy on a warm one, and something else again on the
   * next design somebody makes. A style that says pineapple has to mean it
   * on every board that picks it.
   *
   * Baking asks nothing. The card is painted in the colour and goes through
   * the same paintCard as every other - same edge, same sheen step at the
   * hinge, same grunge - so what flips past is a card that happens to be
   * pineapple, on any design at all.
   */
  cardWash: null,
  /**
   * Keep each card's own character when washing it.
   *
   * Off is right when the colour is the whole gesture: a card wearing a
   * colour has nothing to say and a letter on it is noise. On is for a style
   * *spelled* out of the ring - a creature drawn from `(` and `)` needs its
   * glyphs, or colouring it erases the very thing being coloured.
   */
  washGlyphs: false,
  /**
   * A gesture that goes out and is simply *gone*: no visible journey home.
   *
   * There is no third option here, and it took being wrong twice to see it.
   * The card has to get back to what it was showing, and every way of doing
   * it visibly breaks the rule the style exists for:
   *
   *  - washed on the way home, and you see the three colours again in
   *    reverse, which reads as the card spinning rather than as a gesture;
   *  - unwashed on the way home, and you see the *letters* on the cards it
   *    passes through, which is worse - the whole point was colour, not text;
   *  - and it cannot simply stay put, because then the board is wrong.
   *
   * So the return is not animated at all. Three coloured cards go past, they
   * hold, and the card is back. On a board whose every other movement is a
   * flip this is a deliberate cheat, and the only one that leaves the rule
   * intact.
   */
  vanishHome: false,
  /**
...
export const FIDGET_STYLES = Object.freeze({
  ...
  rainbow: Object.freeze({
    ...
    sweepEvery: 0,
    restOdds: 2,
    // The same three-card gesture as pina colada, quicker and hotter. Three
    // is the rule for every colour style now, not a property of the drink.
    stepDistance: 2,
    stepMs: 190,
    holdMs: 240,
    returnStepMs: 190,
    cardWash: Object.freeze(['#e0503f', '#e8a33c', '#4a90d9']),
    vanishHome: true,
  }),
  /**
  ...
  'pina-colada': Object.freeze({
     ...
     *
     * Real cards, not a colour over the top of one - `cardWash`, not
     * `flight`. Three attempts went through flight first and every one was
     * wrong the same way: a tint on a near-black card comes back grey, and a
     * cover comes back a rectangle with no hinge and no sheen. A card is
     * baked, so a coloured card has to be baked too.
     *
     ...
     */
    /*
     * Two steps, not three, for three colours - and the off-by-one is worth
     * spelling out because it is not a mistake.
     *
     * A tile in flight shows two faces, the one it is leaving and the one it
     * is arriving at, so a journey of N steps puts N+1 cards in front of you.
     * Three steps showed four, and with a three-colour list indexed by ring
     * state the fourth was the first again - a pineapple, a coconut, a lime,
     * and then another pineapple. Two steps is three faces is three colours,
     * once each.
     */
    stepDistance: 2,
    stepMs: 340,
    ...
    holdMs: 420,
    returnStepMs: 340,
    cardWash: Object.freeze(['#f2d16b', '#f7f0e0', '#7fbf5f']),
    /*
     * And the colour is worn on the way out only. Coming home it is the
     * board's own cards again, so the drink is three cards and then it is
     * over - rather than three, a pause, and three more going backwards,
     * which read as the card spinning rather than as a gesture.
     */
    vanishHome: true,
  }),
  /**
   ...
   *
   * Same three-card rule as the other colour styles, just not lingered over:
   * three pastel cards go past quickly enough to be a glimpse.
   */
  sherbet: Object.freeze({
    ...
    stepDistance: 2,
    /*
     * 200ms is not a pause, it is just long enough for the outbound two
     * steps to land - at 60 the gesture was cut off inside its first card
     * and the colour never arrived at all. Then home quickly: the three go
     * past, and that is the whole of it.
     */
    stepMs: 90,
    holdMs: 200,
    returnStepMs: 40,
    cardWash: Object.freeze(['#f7b8c4', '#bfe3c0', '#cfc0e6']),
    vanishHome: true,
  }),
  /**
  ...
  snake: Object.freeze({
    ...
    shortestPath: true,
    traveller: 'snake',
    cardWash: Object.freeze(rampFlight(['#1f5c2a', '#4f9e4a', '#9fd98a'], RING.length)),
    washGlyphs: true,
  }),
  ...
  'pac-man': Object.freeze({
    ...
    shortestPath: true,
    traveller: 'pac-man',
    cardWash: Object.freeze(['#d8b25a', '#e0503f', '#4a90d9', '#e08b3f']),
    washGlyphs: true,
  }),
