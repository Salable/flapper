...
export function dueInterrupters(presets, nowMs, ctx = {}) {
  ...
  (presets ?? []).forEach((preset, index) => {
    ...
    // Nothing to show is nothing to fire. A blank interrupter is a legal
    // thing to have saved ("+ Interrupt" makes one), and the clock quietly
    // putting a blank board up at 5pm is not what anybody scheduled.
    const blank =
      preset.animation === undefined &&
      (Array.isArray(preset.rows)
        ? !preset.rows.some((row) => String(row ?? '').trim() !== '')
        : String(preset.text ?? '').trim() === '');
    if (blank) return;
