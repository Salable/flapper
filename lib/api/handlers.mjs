...
export async function createBoard(request, { broker, db, getSession }) {
  return handle(async () => {
    ...
    /*
     * A card size becomes a grid here, once, at the only moment a board has
     * none - so nothing downstream has to know how to turn one into the other,
     * and no column count is written down in a template. A template that set
     * cols or rows outright keeps them: an odd grid is a thing a board is
     * allowed to want, it just should not be how the ordinary ones are made.
     */
    if (config.cardSize !== undefined && (config.cols === undefined || config.rows === undefined)) {
      const grid = gridFor(config.cardSize, config.screen);
      if (config.cols === undefined) config.cols = grid.cols;
      if (config.rows === undefined) config.rows = grid.rows;
    }
