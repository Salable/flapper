/**
 * The MCP face of a board: tool definitions and the bearer-key verifier for
 * POST /api/b/{slug}/mcp.
 *
 ...
 * `node --test`.
 *
 * Auth is the board's existing API key, presented as the MCP bearer token.
 * `verifyBoardKey` gates the transport once; each tool then re-presents the
 * key to its handler, whose own gate runs identically. Owner-session-only
 * operations (rename, delete, key rotation) are deliberately not tools.
 */
...
display board - a grid of mechanical flip tiles on a wall screen. Board
...
/** The slug embedded in the MCP endpoint's own URL, or null. */
export function mcpSlugOf(request) {
  const match = new URL(request.url).pathname.match(/^\/api\/b\/([^/]+)\/mcp$/);
  return match ? decodeURIComponent(match[1]) : null;
}

/**
 * withMcpAuth verifier: the bearer token must be the API key of the board
 * named in the endpoint path. An unknown board and a wrong key are the same
 * `undefined` (a 401 challenge) - the MCP transport has no 404 to offer.
 */
export async function verifyBoardKey(db, request, bearerToken) {
  if (!bearerToken) return undefined;
  const slug = mcpSlugOf(request);
  const board = slug && (await boards.getBySlug(db, slug));
  if (!board) return undefined;
  if (!(await secretsMatch(bearerToken, board.apiKey))) return undefined;
  return {
    token: bearerToken,
