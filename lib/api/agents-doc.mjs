...
/**
 * The fidgets a board may be told to do, ready to drop into the guide.
 *
 * Built once, out here, rather than inside the template: the doc is one long
 * template literal with its own backticks escaped, and a nested one for the
 * code ticks around each id is a good way to produce a document that throws
 * a 500 instead of a page.
 */
const FIDGET_LIST = Object.keys(FIDGETS)
  .map((id) => '`' + id + '`')
  .join(', ');

/** The card sizes, same treatment - the grid follows from one of these. */
const CARD_SIZE_LIST = CARD_SIZE_IDS.map((id) => '`' + id + '`').join(', ');
...
\`422\`: a board fills whatever window it is in, so what you choose is the
shape of the screen it is designed for (\`screen\`, any two positive numbers -
only the ratio matters) and how big the cards are (\`cardSize\`: ${CARD_SIZE_LIST},
biggest first). The grid follows from those. Either resets with \`null\`.

Changing them re-lays out
...
- \`fidget\` — one of ${FIDGET_LIST}, or \`null\` for the quiet one. Naming one that does not ship is a \`422\` listing
  the ones that do.
- \`ambientMs\` — \`0\` is off, which is the default; anything else is on. **How
  often a fidget happens is part of the fidget**, so the number itself no
  longer means a duration.

A fidget lands on any card of the grid, blank ones included, and puts the
board back exactly as it found it. It never paints over a message that
arrives mid-gesture. Turning one on is a visible change to somebody's wall;
...
saved interrupters can never break each other out of order - including a
scheduled one, whose clock obeys the same rule and waits rather than
demoting what outranks it; a raw
...
### A saved interrupter can start itself

Give one a \`schedule\` and nothing has to call \`.../fire\`:

    {"name": "closing", "text": "WE ARE CLOSED", "durationMs": 60000,
     "schedule": {"kind": "daily", "at": "17:00"}, "timezone": "Europe/London"}

Kinds are \`interval\`, \`everyN\`, \`hourly\`, \`daily\`, \`weekly\`, \`once\`.
\`durationMs\` stops being optional - the window has to close on its own,
since nobody is there at 5pm to dismiss it - and it must be shorter than
the gap between occurrences, or each would still be showing when the next
fires. Nothing polls: a queue read is the moment a window is noticed, and
the occurrence is stamped on the preset, so the same 5pm never starts
twice however many displays are open.

### An animation instead of words

An item's content is text **or** an animation, never both - sending the
pair is a \`422\`. \`{"animation": "explosion"}\` works on \`/message\`, on
\`PATCH ${api}/queue/items/{id}\`, on a saved interrupter, and on a pushed
slide. \`explosion\`, \`rainbow\` and \`bounce\` ship today. It is colour rather
than characters, and it loops for as long as the item is on the glass, so
the dwell decides how long it runs.

### Filling a named slide

\`POST ${api}/sheets/{name}\` puts \`text\`, \`rows\` or \`animation\` into the
slide whose rail name matches, case-insensitively. It fills one that
exists and never creates one - an unknown name is a \`404\` naming the
slides that are there - and the slide's Hold, alignment and place in the
...
| \`PATCH\` | \`/config\` | key | \`screen\`, \`cardSize\`, \`theme\`, \`themePack\`, \`fidget\`, \`ambientMs\`, dwell (\`footerRows\` must stay 0) |
