...
\`${base}/api/mcp\` — one endpoint for every board (Streamable HTTP). The
bearer token is the board API key, and **the key names the board**: connect
with this board's key and every tool drives this board. The tools mirror the
endpoints below — if your client can hold an MCP connection, prefer it over
hand-rolled HTTP:
...
- **claude.ai / Claude Desktop**: Settings → Connectors → *Add custom
  connector* → \`${base}/api/mcp\`, with the key as an \`authorization\`
  request header (\`Bearer ${KEY}\`).
- **ChatGPT**: developer mode → add connector → \`${base}/api/mcp\`,
  auth = bearer token.
