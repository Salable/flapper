...
const STARTERS = [
  {
    ...
    params: {},
    /*
     * A queue of one, which is what a sign is.
     *
     * Said in the config rather than left to the template id, because the
     * board has to go on being a sign after the template has done its work.
     * Everything that makes a queue a queue - waiting, ordering, jumping
     * ahead - needs something to wait behind, so at a cap of one none of it
     * applies and the panel does not offer it.
     */
    config: { queueCap: 1 },
