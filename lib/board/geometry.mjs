...
/**
 * Whether a board is a sign: a queue capped at one, so posting always
 * replaces rather than adds. Derived from the cap, not the type id or the
 * template - a board that has raised or dropped its cap is called what it
 * currently is, not what it was made as. One home for this test so
 * QueueManager, BoardSidebar and SettingsClient can't drift.
 */
export function isSignConfig(config = {}) {
  return (Number(config?.queueCap) || Infinity) === 1;
}
