...
export async function resetTestDb(db) {
  await db.execute(
    sql.raw(
      'TRUNCATE "licence_requests", "designs", "queue_items", "boards", "queues", "session", "account", "verification", "user", ' +
        '"jwks", "oauth_client", "oauth_resource", "oauth_client_resource", ' +
        '"oauth_refresh_token", "oauth_access_token", "oauth_consent", "oauth_client_assertion" CASCADE',
    ),
  );
}
