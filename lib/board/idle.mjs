...
const FIDGET_DEFAULTS = Object.freeze({
   ...
   * within that many steps *either way*, which is the cheat that makes a
   * cheap gesture look like an arbitrary one. An exact distance does not:
   * at exactly one step, every blank card ticks to A and nothing else, and
   * since most of a board is blank that is most of what you ever see - 78%
   * of them, measured. A radius of four gives a blank card eight characters
   * to choose from and still costs at most eight flips there and back.
   */
  stepDistance: 0,
