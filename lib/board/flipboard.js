...
export const DEFAULTS = {
  ...
  // Colours to bake whole cards in while they move - see `washCards()`.
  // null is the design's own cards, which is every board unless a fidget
  // has borrowed this for the length of a gesture.
  cardWash: null,
  // Keep each card's own character when washing it. Off paints them blank.
  cardWashGlyphs: false,
  // Flat indices that wear the wash even at rest, or null for none. A moving
  // card is washed anyway; this is for the pause at the far end of a gesture,
  // where the card has landed and is still meant to be showing a colour.
  cardWashCells: null,
  // Device pixels per CSS pixel. null reads the browser's own; an offline
  // renderer (tools/export-video.mjs) sets it, having no window to ask.
  pixelRatio: null,
...
/** How many distinct wash colours to keep baked at once. Each is a canvas
 * per ring state, so this is the cap that matters for memory. */
const WASH_CACHE_MAX = 48;
...
export class Flipboard {
  ...
  /**
   * What one band is showing, or null if it has never been painted.
   *
   * `page` stitches every band together, which is right for anything asking
   * "what does the glass say" and wrong for anything that then wants to
   * *write* somewhere - because `setPage` writes the main band alone. Read
   * the whole board, index into it, write back, and on a board with a footer
   * the indices refer to one thing and the write to another.
   */
  regionPage(regionId) {
    const lines = this.regionLines.get(regionId);
  ...
  setSkin(skin) {
    ...
    this.skin = skin;
    this.tileSize = skin.tileSize;
    // Both wash caches are baked against the old skin's own cards.
    this._wash = null;
    this._washKey = null;
    this._washes = null;
    this.draw();
   ...
   */
  resize() {
    // `pixelRatio` for a renderer that has no window - an offline export
    // picks its own (tools/export-video.mjs). The browser stays the default
    // and behaves exactly as before.
    const dpr =
      this.opts.pixelRatio || (typeof window === 'undefined' ? 1 : window.devicePixelRatio || 1);
  ...
  draw() {
    ...
    const wash = this.washCards(size);
    /*
     * `cardWashCells` is either flat indices (a fidget: these cells, the one
     * colour in `cardWash`) or `{index, colour}` entries (an animation: this
     * cell, this colour). Both end up as a Set of indices for the "is this
     * card washed at all" test; the second also builds the per-cell colour
     * map the draw loop reads below.
     */
    const washGlyphs = Boolean(this.opts.cardWashGlyphs);
    let washCells = null;
    let washColours = null;
    if (Array.isArray(this.opts.cardWashCells)) {
      washCells = new Set();
      for (const entry of this.opts.cardWashCells) {
        if (typeof entry === 'number') {
          washCells.add(entry);
          continue;
        }
        if (!entry || typeof entry.index !== 'number') continue;
        washCells.add(entry.index);
        if (typeof entry.colour === 'string') {
          if (!washColours) washColours = new Map();
          washColours.set(entry.index, entry.colour);
        }
      }
    }
    ...
    for (let row = 0; row < rows; row += 1) {
      for (let col = 0; col < cols; col += 1) {
        ...
        /*
         * A moving card wears the wash; so does one named in `cardWashCells`.
         *
         * In-flight-only was the rule at first and it was wrong in a way that
         * took a screenshot to see: a gesture that goes out, *pauses*, and
         * comes back spends its whole pause landed, so the card blinked back
         * to the design's own paint - letter and all - in the middle of what
         * was supposed to be three colours and no text. The pause is the part
         * you actually look at.
         *
         * Named cells rather than "everything at rest", because the wash is a
         * board-wide option: washing every landed tile would repaint the
         * message underneath it too.
         */
        const index = row * cols + col;
        /*
         * Named cells only, in flight as well as at rest.
         *
         * "In flight or named" was the first rule, and the in-flight half of
         * it was unrestricted - so while a fidget held a colour beat, *any*
         * tile that started moving wore the colour too. A message arriving
         * mid-gesture flipped in as solid blank cards for the length of a
         * beat. A wash belongs to the cards the gesture is using and to
         * nothing else; a caller wanting the whole board can name every cell.
         */
        const mine = washCells === null || washCells.has(index);
        // This cell's own colour wins over the board-wide palette; an
        // animation names one per cell, a fidget names none and gets `wash`.
        const own = washColours ? this.washFor(size, washColours.get(index), washGlyphs) : null;
        const set = own ?? wash;
        const worn = set && mine && (tile.progress > 0 || washCells !== null) ? set : undefined;
        /*
        ...
        if (tile.dir === -1 && tile.progress > 0) {
          ...
          skin.drawTile(ctx, below, 1 - tile.progress, tx, ty, size, worn);
        } else {
          skin.drawTile(ctx, tile.state, tile.progress, tx, ty, size, worn);
        }
        ...
        if (flight && tile.progress > 0) {
          ...
          if (flash) {
            ctx.save();
            /*
             * `overlay` keeps the glyph readable and takes its brightness
             * from the card underneath, which is right for a pattern caught
             * on the way past. It is not a way to make a card *be* a colour
             * - on a dark design it returns grey - and it was briefly made
             * configurable in the hope that it was. It is not; a coloured
             * card is a baked card. See `cardWash`.
             */
  ...
  /**
   * This board's washed cards, baked on demand and kept until the palette or
   * the tile size changes.
   *
   * Kept here rather than on the skin deliberately. A skin is shared - one
   * per tab, behind the wordmark and every board at once - so a board that
   * wants its cards a different colour for a moment must own that set, or
   * it would repaint everybody else's board too.
   */
  washCards(size) {
    const palette = this.opts.cardWash;
    if (!palette || !this.skin.bakeWash) {
      this._wash = null;
      this._washKey = null;
      return null;
    }
    const glyphs = Boolean(this.opts.cardWashGlyphs);
    const key = `${size}:${glyphs}:${palette.join(',')}`;
    if (this._washKey !== key) {
      this._wash = this.skin.bakeWash(size, palette, glyphs);
      this._washKey = key;
    }
    return this._wash;
  }
  ...
  /**
   * One baked set per colour, for a frame that wants several at once.
   *
   * `cardWash` is a palette spread across the *ring states* (see a skin's
   * `bakeWash`), which is the right shape for a fidget: one colour a beat,
   * every card the same. An animation is the other shape - the picture *is*
   * the colour, so two cards a tile apart are two different colours in the
   * same frame - and that cannot be said in a single baked set.
   *
   * So: bake one set per distinct colour and cache them by colour. A frame
   * uses as many as it names, and asking again for one already baked costs
   * a map lookup. Cleared with the rest when the size or skin changes.
   */
  washFor(size, colour, glyphs) {
    if (!colour || !this.skin.bakeWash) return null;
    const key = `${size}:${glyphs}:${colour}`;
    if (!this._washes) this._washes = new Map();
    const hit = this._washes.get(key);
    if (hit) {
      // Least-recently-used, cheaply: re-inserting moves it to the back of
      // the Map's own insertion order, which is what the eviction below
      // walks. Same trick the theme cache upstairs would want.
      this._washes.delete(key);
      this._washes.set(key, hit);
      return hit;
    }
    const baked = this.skin.bakeWash(size, [colour], glyphs);
    this._washes.set(key, baked);
    /*
     * Bounded, because each entry is a canvas *per ring state* - forty-two
     * of them - and the cache is keyed by colour. An animation sweeping a
     * continuous hue would otherwise bake one set per shade and hold every
     * one for the life of the tab. The animations quantise their palettes
     * for the same reason (HUE_STEP in lib/board/animations.mjs); this is
     * the backstop for anything that does not.
     */
    while (this._washes.size > WASH_CACHE_MAX) {
      const oldest = this._washes.keys().next().value;
      this._washes.delete(oldest);
    }
    return baked;
  }
  ...
  start() {
    ...
    this.lastTick = typeof performance === 'undefined' ? 0 : performance.now();
    this.schedule();
  }

  schedule() {
    if (this.raf !== null) return;
    // An offline renderer calls tick() itself with the timestamps it wants,
    // so there is nothing to schedule and nothing to cancel - rather than
    // making every caller stub a browser global that only exists to hand
    // over a number the exporter already has.
