...
export const user = pgTable('user', {
  ...
  image: text('image'),
  /** Read by nothing. Salable answers what an account may do, at request
   * time (lib/salable/licence.mjs) - there is no tier here to go stale. Due
   * to be dropped once every account holds a licence; the column is left in
   * place until then so a rollback has somewhere to land. */
...
/**
 * A design somebody made: a theme pack with a name, belonging to an account.
 *
 * The five that ship live in lib/board/themes.mjs, because they are part of
 * the build and a typo in one should fail the test run rather than a wall.
 * These are the other kind - made in the designer, or by an agent posting a
 * pack to /api/designs, which is the same door and the same validator.
 *
 * The pack is stored whole rather than as a diff against a preset. A board's
 * override is a diff because it is an edit *of* something; a design is a thing
 * in its own right, and a preset it happened to start from is history, not
 * structure.
 */
/**
 * A refusal that turned into a conversation.
 *
 * Every paid plan is bespoke, so there is no checkout to send anyone to and
 * no price list to read: the end of a 402 is a person. This table is that
 * queue. It is deliberately small - what they hit, what they said, how to
 * reach them - because the point is to reply, not to run a CRM out of
 * Postgres.
 *
 * `need` is one of lib/salable/licence.mjs REQUESTABLE, which is the same
 * vocabulary the 402 carries, so a request arrives already sorted into the
 * entitlement a bespoke plan would grant.
 */
export const licenceRequests = pgTable(
  'licence_requests',
  {
    id: text('id').primaryKey(),
    userId: text('user_id')
    ...
    /** What they said they needed it for. Their words, never ours. */
    message: text('message').notNull(),
    /** A better address than the account's, when they gave one. */
    contact: text('contact'),
    /** Set when a person has actually replied - not when the row was read. */
    handledAt: timestamp('handled_at'),
    createdAt: timestamp('created_at').notNull().defaultNow(),
  },
  (table) => [index('licence_requests_user_idx').on(table.userId)],
);
...
export const designs = pgTable(
  'designs',
  {
    ...
    ownerId: text('owner_id')
      ...
      .references(() => user.id, { onDelete: 'cascade' }),
    name: text('name').notNull(),
    /** A validated theme pack (lib/board/theme-pack.mjs). */
    pack: jsonb('pack').notNull(),
    /** Which shipped preset it was forked from, for the record. Never resolved. */
    basedOn: text('based_on'),
    createdAt: timestamp('created_at').notNull().defaultNow(),
    updatedAt: timestamp('updated_at').notNull().defaultNow(),
  },
  (table) => [index('designs_owner_idx').on(table.ownerId)],
);
