...
/**
 * A saved interrupter: a name, its text, and the Duration it fires with.
 * `durationMs` is one or the other, not both - a hard limit (shown, then
 * gone outright, whichever comes first between its own turn ending and
 * the limit) if given; the switch (blocks the rotation entirely until
 * dismissed or broken by a higher-ranked one) if absent. Text is checked
 * to the same limit a live message's own text carries - firing one posts
 * exactly this body through the same door `POST /message` does, so
 * nothing saved here could be refused there.
 */
export function validateInterrupterPreset(body, limits = LIMITS) {
  ...
  if (typeof body.text !== 'string' || body.text.trim() === '') {
    reject('text is required and must be a non-empty string', 422);
  }
  if (body.text.length > limits.maxTextLength) {
    reject(`text exceeds ${limits.maxTextLength} characters`, 413);
  }
  const preset = { name, text: body.text };
