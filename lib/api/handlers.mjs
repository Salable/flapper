...
import { sharedLicence } from '../salable/licence.mjs';
 ...
 * from the account's allowance instead - `boards:unlimited` means unlimited,
 ...
 */
const GET_IN_TOUCH = 'this needs a plan we don’t sell off the shelf; get in touch';

function refuseUnlicensed(what, need) {
  reject(`${what}. This account holds no Flapper licence - sign out and back in, or contact support (${need})`, 403);
}
...
export async function createBoard(request, ctx) {
  ...
  return handle(async () => {
     ...
     * all, how many boards does it cover, and is this type included.
     */
    const allowance = await allowanceFor(ctx, session.user.id);
    if (!allowance.licensed) {
      refuseUnlicensed('a board needs a licence', 'board.create');
    }
    ...
    if (held >= ceiling) {
      ...
      reject(
        `this licence covers ${ceiling} board${ceiling === 1 ? '' : 's'} and ${held} ${held === 1 ? 'is' : 'are'} in use; delete one, or ${GET_IN_TOUCH} (boards)`,
        402,
      );
    }
    ...
    if (type.entitlement && allowance.types && !allowance.types.includes(type.id)) {
      reject(
        `"${type.name}" boards are not on this licence - ${GET_IN_TOUCH} (${type.entitlement}). A live queue board is included.`,
        402,
      );
    }
  ...
  return handle(async () => {
    ...
    if (body.private === true && !board.private) {
      ...
      if (!allowance.privateBoards) {
        reject(
          `private boards are not on this licence - ${GET_IN_TOUCH} (board.private). This board stays readable by anyone with its URL.`,
          402,
        );
      }
    }
 ...
 */
export async function saveInterrupter(request, { db, slug, getSession }) {
  return handle(async () => {
    const board = await requireBoardBySlug(db, slug);
    await requireKeyOrOwner(request, board, getSession);
    const body = await readJsonBody(request, LIMITS.maxBodyBytes);
