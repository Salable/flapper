...
/**
 * Leftmost and rightmost columns pan this far out, -1..1. Subtle on purpose:
 * a listener stands in front of the middle of a wall, so a flap on the far
 * left should read as "over there" rather than hard in one ear. 0.35 is
 * about 20 degrees off-centre on an equal-power panner.
 */
export const PAN_WIDTH = 0.35;
 ...
 * the right, 0 for a one-column board. The curve is gentle through the
 * middle - the central third of a board sits almost dead centre and the
 * image only opens out toward the edges - so a message that fills the glass
 * reads as one wide board, not a ping-pong.
 */
export function panForColumn(col, cols) {
  if (cols <= 1) return 0;
  const x = (col / (cols - 1)) * 2 - 1;
  return Math.sign(x) * Math.abs(x) ** 1.5 * PAN_WIDTH;
}
