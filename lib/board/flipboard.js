 ...
 * same 42-state cycle (blank, A-Z, 0-9, . , ! ( ) ) and the same set of 42
 * sprite strips, where `strips[i]` animates state `i` into state `i + 1`.
 *
 * Frame 0 of a strip is its source state at rest and frame 9 is its
 * destination at rest, and the build step guarantees frame 9 of `strips[i]` is
 * byte-identical to frame 0 of `strips[i + 1]`. So:
 *
 *   at rest on state i  -> strips[i], frame 0
 *   flipping i -> i + 1 -> strips[i], frames 1..9
 *
 * and a tile coming to rest never shifts a pixel.
 *
...
export class Flipboard {
   ...
   */
  constructor(canvas, manifest, strips, options = {}) {
    this.canvas = canvas;
    this.ctx = canvas.getContext('2d', { alpha: false });
    this.strips = strips;
    this.cycle = manifest.cycle;
    this.states = manifest.cycle.length;
    ...
    if (this.blank < 0) throw new Error('manifest has no blank state');
    this.charset = charsetFromManifest(manifest);
  ...
  /**
   * Swap the tile art under the board - a theme change. The new set must be
   ...
   */
  setArt(manifest, strips) {
    if (manifest.cycle.length !== this.states) {
      throw new Error(`tile art has ${manifest.cycle.length} states, board has ${this.states}`);
    }
    manifest.cycle.forEach((state, i) => {
      if (state.char !== this.cycle[i].char) {
        throw new Error(`tile art state ${i} is ${JSON.stringify(state.char)}, expected ${JSON.stringify(this.cycle[i].char)}`);
      }
    });
    this.strips = strips;
    this.tileSize = manifest.tileSize;
  ...
  makeTile() {
    return { state: this.blank, pending: 0, frame: 0, elapsed: 0, wait: 0 };
  }
  ...
  tick(now) {
    ...
    for (const region of this.regions) {
      ...
      for (let index = region.start; index < region.end; index += 1) {
        ...
        if (tile.pending > 0) {
          const duration = this.stepDuration(tile.pending);
          const progress = Math.min(0.999999, tile.elapsed / duration);
          tile.frame = 1 + Math.floor(progress * (this.frames - 1));
        } else {
          tile.frame = 0;
          tile.elapsed = 0;
        }
