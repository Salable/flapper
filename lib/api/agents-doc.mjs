...
This board also speaks the Model Context Protocol at \`${api}/mcp\`
(Streamable HTTP). The bearer token is the same board API key, and the tools
mirror the endpoints below — if your client can hold an MCP connection, prefer
it over hand-rolled HTTP:

- **Claude Code**: \`claude mcp add --transport http ${slug} ${api}/mcp --header "authorization: Bearer ${KEY}"\`
- **claude.ai / Claude Desktop**: Settings → Connectors → *Add custom
  connector* → \`${api}/mcp\`, with the key as an \`authorization\` request
  header (\`Bearer ${KEY}\`).
- **ChatGPT**: developer mode → add connector → \`${api}/mcp\`,
  auth = bearer token.

The MCP surface is the API-key surface: no board management, no display
calls. Everything in this document about the charset, the queue, and status
codes applies to the tools identically.
...
| \`POST\` | \`/mcp\` | key | the MCP endpoint (Streamable HTTP; see section 1) |
