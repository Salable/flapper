...
import { resolveBoardTheme } from '../board/board-theme.mjs';
import { gridForConfig } from '../board/geometry.mjs';
...
function headlessBoard(tiles, config = {}) {
  const charset = charsetFromManifest(tiles);
  const opts = {
    cols: DEFAULTS.cols,
    rows: DEFAULTS.rows,
  ...
  const board = {
    ...
    supportedChars() {
      return tiles.cycle.map((state) => state.char);
    },
  };
  return board;
}
 ...
 */
export function headlessController(tiles, config = {}) {
  const board = headlessBoard(tiles);
  const controller = new Controller(board, {});
  /*
   * The grid, worked out from the screen and the card size - a config never
   * carries one, so anything that needs to lay a board out computes it. How
   * the board moves - Hold, Travel speed, Landing, Sweep, Sweep shape,
   * Full revolution - is the same story: it lives on the design's pack now, not
   * the config, so it is resolved here the same way the display resolves it.
   */
  const patch = {
    ...config,
    ...gridForConfig(config),
    ...resolveBoardTheme(config).pack.advanced,
  };
