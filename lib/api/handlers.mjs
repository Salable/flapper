...
import { dueInterrupters } from '../board/interrupt-schedule.mjs';
...
/**
 * Start any scheduled interrupter whose window has just opened.
 *
 * Called on a live board's queue read, which is the only moment a live
 * board has (see `sweepExpiredLive` above for the same reasoning). The
 * occurrence instant is stamped onto the preset as `firedForMs` inside the
 * same row-locked read-modify-write the save path uses, so the stamp is
 * what makes this idempotent: two displays reading on the same millisecond
 * both compute the same 5pm, and only the first to take the lock sees it
 * as unfired.
 *
 * @returns whether anything was admitted, so the caller can re-read.
 */
async function fireDueInterrupters({ broker, db, board, type }) {
  const nowMs = Date.now();
  if (dueInterrupters(board.config?.interrupters, nowMs, { createdAtMs: board.createdAt?.getTime?.() ?? 0 }).length === 0) {
    return false;
  }

  // Rank, the same rule a hand-fire obeys: a lower-ranked interrupter can
  // never break a higher-ranked one that is showing. The clock used to walk
  // straight past this and demote a live fire alarm for a lunch menu, which
  // a `.../fire` call at the same moment would have refused with a 409.
  const { items, currentItemId } = await queue.listQueue(db, board.id);

  // Re-decided under the lock rather than trusting the read above: the
  // list may have been saved, reordered, or fired by somebody else in
  // between, and `firedForMs` is only worth anything if the decision and
  // the stamp are the same transaction.
  let toFire = [];
  await boards.updateInterrupters(db, board.id, (existing) => {
    toFire = dueInterrupters(existing, nowMs, { createdAtMs: board.createdAt?.getTime?.() ?? 0 }).filter(
      ({ index }) => !outrankedBy(existing, items, currentItemId, index),
    );
    if (toFire.length === 0) return existing;
    const next = existing.map((preset) => ({ ...preset }));
    for (const { index, occurrenceMs } of toFire) next[index].firedForMs = occurrenceMs;
    return next;
  });
  if (toFire.length === 0) return false;

  // Lowest-ranked first, so the highest-ranked one ends up on the glass -
  // `priority: 'now'` puts each admitted item at the head, and order in
  // this list is the ranking (docs/QUEUES.md).
  const failed = [];
  for (const { preset, index, occurrenceMs } of [...toFire].reverse()) {
    try {
      await admit(db, board, type, interrupterBody(preset, { occurrenceMs, nowMs }), 'schedule');
    } catch (error) {
      // One bad preset must not cost the others their turn, and must not
      // fail the read that noticed it - the display still wants its queue.
      console.error(`flapper: scheduled interrupter "${preset.name}" on ${board.slug} - ${error.message}`);
      failed.push({ index, name: preset.name });
    }
  }

  /*
   * A stamp is a record of having fired, so one that did not fire is taken
   * back. Stamping before admitting is what makes two simultaneous reads
   * safe, but it also meant a preset whose admission threw was marked as
   * done and never tried again - one bad afternoon and that interrupter is
   * silently retired for good. Undone only where the name still matches, so
   * a save that landed in between keeps its own stamp.
   */
  if (failed.length > 0) {
    await boards.updateInterrupters(db, board.id, (existing) => {
      const next = existing.map((preset) => ({ ...preset }));
      for (const { index, name } of failed) {
        if (next[index]?.name === name) delete next[index].firedForMs;
      }
      return next;
    });
  }
  ...
  await nudge(broker, db, board.id);
  return toFire.length > failed.length;
}

/* ---- the queue itself ---- */

export async function getQueue(request, { broker, db, slug, getSession }) {
  return handle(async () => {
    const board = await requireBoardBySlug(db, slug);
    await requireReadAccess(request, board, getSession);
    const type = getBoardType(board.type);
    ...
    if (type && type.playback === 'clock') {
      ...
      // ...and the same moment is when a scheduled interrupter's window is
      // noticed. The server decides that 5pm has arrived, so a display
      // still only renders what it is told and two screens cannot each
      // decide it separately (see lib/board/interrupt-schedule.mjs).
      if (await fireDueInterrupters({ broker, db, board, type })) {
        snapshot = await queue.listQueue(db, board.id);
      }
    }
...
/**
 * Push content into a slide by its own name - the "Pushed" source, at last.
 *
 * A slide is addressable by id already (`PATCH /queue/items/{id}`), which
 * is no use to an agent: ids are minted by the server and change when a
 * slide is remade. A slide's *name* is the thing a person and an agent can
 * agree on in advance - "the lunch one" - and it is already on the item as
 * `options.label`, which the rail shows as the tab. So this resolves the
 * name and patches the payload, and nothing new has to be stored.
 *
 * Deliberately not an upsert. `POST /interrupters` creates by name because
 * an interrupter is a saved thing with no place in the rotation until it
 * fires; a slide is a position in the queue, and one appearing because an
 * agent typed a name that did not exist yet would be a board rearranging
 * itself over a typo. The slide is made in the control room (or by
 * `/message`), and this fills it.
 *
 * Content only - what the slide *is*. Its Hold, its order and whether it
 * loops belong to whoever arranged the rotation, and stay where they are.
 */
export async function pushSheet(request, { broker, db, slug, name, getSession }) {
  return handle(async () => {
    const board = await requireBoardBySlug(db, slug);
    await requireKeyOrOwner(request, board, getSession);
    requireType(board);
    const body = await readJsonBody(request, LIMITS.maxBodyBytes);
    if (body.text === undefined && body.rows === undefined && body.animation === undefined) {
      reject('nothing to show; this route takes text, rows, or animation', 422);
    }

    const wanted = String(name ?? '').trim().toLowerCase();
    // A blank name is not an address. Without this it matched the first
    // slide with no name of its own - and "+ Slide" makes those - so a
    // caller with an empty path segment quietly overwrote a stranger's
    // untitled slide.
    if (wanted === '') reject('a slide name is required in the path', 404);
    const { items } = await queue.listQueue(db, board.id);
    const target = items.find((item) => String(item.payload?.options?.label ?? '').trim().toLowerCase() === wanted);
    if (!target) {
      const named = items
        .map((item) => String(item.payload?.options?.label ?? '').trim())
        .filter((label) => label !== '');
      reject(
        named.length > 0
          ? `no slide called "${name}" on this board; it has ${named.map((label) => `"${label}"`).join(', ')}`
          : `no slide called "${name}" on this board, and none of its slides are named yet`,
        404,
      );
    }

    /*
     * Content replaces content; everything else about the slide stays.
     *
     * The first pass built the entry from the pushed body alone, which
     * silently dropped the slide's Hold, its alignment, its label and its
     * interrupt flag - the exact opposite of what this route's own docblock
     * promises, and a push meant to change the words would quietly re-cut
     * the timing of a rotation somebody had arranged.
     *
     * So: start from what the slide already is, take out the three content
     * fields (they are alternatives - leaving the old ones in would send
     * text *and* an animation, which the validator rightly refuses), and
     * lay the pushed content over the top. The label rides along from the
     * existing payload, so the address survives whatever the body says -
     * renaming is the rail's own job, not a side effect of a push.
     */
    const kept = { ...(target.payload?.options ?? {}) };
    delete kept.rows;
    delete kept.animation;
    ...
    const patch = { payload: entry.payload };
    // A clock board's slot length is a function of its payload, so new
    // content means a new slot - the same recompute patchQueueItem does.
    const type = getBoardType(board.type);
    if (type?.playback === 'clock') Object.assign(patch, clockTiming(board, entry.payload, target.schedule));
    const updated = await queue.updateItem(db, board.id, target.id, patch);
    await nudge(broker, db, board.id);
    return json(200, { ok: true, item: publicItem(updated) });
  });
}
  ...
  return handle(async () => {
    ...
    const config = await boards.updateInterrupters(db, board.id, (existing) => {
      ...
      if (index >= 0) {
        ...
        /*
         * `firedForMs` survives a re-save, unless the schedule itself changed.
         *
         * A save replaces the stored preset outright (editing is re-saving),
         * and the validator builds a fresh whitelist from the body - so the
         * stamp, which no caller ever sends, was being erased. The cost was
         * the whole idempotence guarantee: fixing a typo at 17:05 dropped the
         * stamp, the next queue read saw today's 17:00 still unfired, and a
         * second copy went straight to the glass. Every edit inside the
         * window re-fired it.
         *
         * A *changed* schedule is a different series, so its old occurrence
         * means nothing and the stamp goes - which is also what lets somebody
         * re-point an interrupter at an earlier time today and have it fire.
         */
        const before = existing[index];
        const sameSeries = sameSchedule(before?.schedule, preset.schedule);
        interrupters[index] =
          sameSeries && typeof before?.firedForMs === 'number'
            ? { ...preset, firedForMs: before.firedForMs }
            : preset;
        return interrupters;
      }
 ...
 */
/**
 * Whether two schedule specs are the same series.
 *
 * Compared key by key, sorted, rather than by stringifying: what comes back
 * out of a jsonb column has its keys in the database's own order, not the
 * one the validator wrote them in, so `JSON.stringify` said "different" for
 * two identical schedules - and dropped the firedForMs stamp this exists to
 * keep.
 */
...
/** Whether a saved interrupter has anything at all to put on the glass. */
function hasContent(preset) {
  if (preset?.animation !== undefined) return true;
  if (Array.isArray(preset?.rows)) return preset.rows.some((row) => String(row ?? '').trim() !== '');
  return String(preset?.text ?? '').trim() !== '';
}

/**
 * The saved interrupter currently on the glass, if it outranks `index`.
 *
 * Order is the only ranking a saved interrupter has and it is enforced: a
 * lower-ranked one can never break a higher-ranked one that is showing
 * (docs/QUEUES.md). Shared, because there are two doors to the glass now -
 * a fire and the clock - and a rule only one of them applies is not a rule.
 *
 * @returns the blocking preset, or null when nothing outranks this one
 */
function outrankedBy(presets, items, currentItemId, index) {
  const current = items.find((item) => item.id === currentItemId);
  if (current?.payload?.options?.interrupt !== true) return null;
  ...
  return currentIndex >= 0 && currentIndex < index ? presets[currentIndex] : null;
}

/**
 * The queue entry a saved interrupter becomes. Shared by the two things
 * that can start one: a fire (by hand or over the API) and the clock, when
 * a scheduled interrupter's window opens.
 */
function interrupterBody(preset, { occurrenceMs = null, nowMs = Date.now() } = {}) {
  ...
  if (preset.animation !== undefined) {
    // An animation carries no words, and textOptions refuses the pair.
    body.text = '';
    body.animation = preset.animation;
  } else if (preset.rows !== undefined) {
  ...
  if (preset.durationMs !== undefined) {
    ...
    //
    // A scheduled one's window opened at its occurrence, not at the moment
    // a read happened to notice - so the remaining time is measured from
    // there. Otherwise a board read a minute late showed a one-minute
    // announcement for a minute *starting then*, drifting a little further
    // every day it was noticed late.
    const elapsed = occurrenceMs === null ? 0 : Math.max(0, nowMs - occurrenceMs);
    const left = Math.max(1, preset.durationMs - elapsed);
    body.dwellMs = left;
    body.expiresInMs = left;
  } else {
...
export async function fireInterrupter(request, { broker, db, slug, name, getSession }) {
  return handle(async () => {
    ...
    const { items, currentItemId } = await queue.listQueue(db, board.id);
    const outranked = outrankedBy(presets, items, currentItemId, index);
    if (outranked) {
      reject(
        `"${outranked.name}" is showing and ranks ahead of "${preset.name}" - move "${preset.name}" above it, or wait for "${outranked.name}"'s own turn`,
        409,
      );
    }

    // Blank text is fine to *save* - "+ Interrupt" makes one before there is
    // anything to type - but firing one with no Duration is the switch, and
    // the switch on a blank interrupter blanks the wall until somebody
    // notices and dismisses it. The engine's "forever" is 24 hours.
    if (!hasContent(preset) && preset.durationMs === undefined) {
      reject(`"${preset.name}" has nothing to show, so firing it would blank the board until it is dismissed - give it text, an animation, or a Duration`, 422);
    }

    const item = await admit(db, board, type, interrupterBody(preset), 'api');
    await nudge(broker, db, board.id);
