...
export class ProceduralSkin {
  ...
  drawTile(ctx, state, progress, x, y, size) {
    ...
    if (inFlight) {
      ...
      // The lighting below - the card under the flap darkening, and the
      // flap itself brightening - is a property of the printed face, not
      // the black edge around it. Clipped to the inset face (the same
      // rounded rect card.fill itself is drawn into), not the full tile:
      // a plain rect spanning the whole tile ignores both the rounding and
      // the edge, and washed a lighter rectangle straight over what should
      // have stayed a flat black bezel - visible as the shading appearing
      // to sit on top of the card rather than inside it.
      const inset = size * card.inset;
      if (ctx.clip) {
        ctx.save();
      ...
      if (ctx.clip) ctx.restore(); // undo the face clip
    }
