 ...
 */
export async function createBoard(db, { ownerId, name = '', slug } = {}) {
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    ...
    const row = {
      ...
      // A board is born with its own live queue, sharing the board's id.
      queueId: id,
    };
    try {
...
export async function updateBoard(db, id, { name, slug, private: isPrivate } = {}) {
