...
/** The longest a caller may ask any one message to dwell - comfortably
 * under setTimeout's own real ceiling (~24.8 days, a signed 32-bit ms
 * count), and long enough that "until dismissed" (an interrupter blocking
 * the rotation until someone removes it, rather than a genuinely
 * unbounded hold this clamp still refuses) reads as indefinite in
 * practice. Was 600000 (10 minutes) - too short for that to hold up. */
...
export class Track {
  ...
  enqueue(text, options = {}) {
    ...
    const message = {
      ...
      dwellMs: clampNumber(options.dwellMs, 0, MAX_DWELL_MS),
  ...
  advance() {
    ...
    this.current = null;
    // A message is done: all pages shown, final dwell served. This is the
    // hook the queue player advances on - distinct from `clear`, which also
    // nulls `current` but is not completion.
    this.onMessageDone?.(message);
  ...
  status() {
    return {
      ...
      // `showing` answers "what is on the glass?" in every state: the message
      // being played, or - once the queue drains - the finished one whose
      // last page stands there. It was null exactly when the board was at
      // rest and most readable, which is when an agent checks. `phase` keeps
      // the distinction (the server queue uses the same words).
      phase: this.current ? 'playing' : this.holding ? 'holding' : 'blank',
