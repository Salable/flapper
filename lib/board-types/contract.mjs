...
export function assertDefinition(definition) {
  ...
  need(
    definition.entitlement === undefined ||
      (typeof definition.entitlement === 'string' && definition.entitlement.length > 0),
    'entitlement is a non-empty string',
  );
}

/**
 * A type that is not free names the entitlement it needs
 * (`entitlement: 'board.type.scheduled'`), and that is the whole of Flapper's
 * side of it: the account either holds that value or does not, and Salable is
 * the only thing that decides. Enforced in createBoard, which REST and the
 * MCP create_board tool share - a greyed-out card is decoration; this is the
 * paywall. See lib/salable/licence.mjs for the vocabulary.
 *
 * There used to be a TIER_LADDER and an entitled() here, comparing a type's
 * tier against user.tier. It gated nothing - no type ever named a tier - and
 * a ladder in the application is the pattern this example exists to replace.
 */
