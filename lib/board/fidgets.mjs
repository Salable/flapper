...
export function validateFidget(spec) {
  ...
  positive('everyMs', 1000, 3_600_000);
  positive('beatMs', 40, 5000);
  positive('cards', 1, 64);
  if (!Number.isFinite(Number(f.varyMs)) || Number(f.varyMs) < 0) {
    errors.push('varyMs must be 0 or a positive number of milliseconds');
  }
  if (Number(f.varyMs) >= Number(f.everyMs)) {
    errors.push('varyMs must be smaller than everyMs, or a gap could be negative');
  }
