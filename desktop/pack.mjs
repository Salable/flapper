#!/usr/bin/env node
/**
 * Package the Flapper desktop shell into a distributable macOS .app bundle,
 * then ad-hoc sign it and zip it with `ditto`.
 *
...
// Whitelist: the shell is three files; everything else stays out.
const KEEP = /^\/(package\.json|main\.js|settings\.js)$/;

const icon = path.join(REPO, 'build', 'icon.icns');
if (!fs.existsSync(icon)) {
  console.error('build/icon.icns is missing. Run: python3 tools/build_icon.py');
  process.exit(1);
}

console.log(`packaging Flapper shell ${pkg.version} for darwin/${arch}`);
