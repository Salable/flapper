...
/**
 * How long between one occurrence and the next, or null when the spec has no
 * period at all (`once`). Read off `triggersOf`, so there is one definition
 * of what a spec means rather than two that can drift.
 */
export function periodOf(schedule) {
  if (!schedule || schedule.kind === 'once') return null;
  const { period } = triggersOf(schedule, { tz: 'UTC', createdAtMs: 0 }, 0);
...
/**
 * How far away an instant is, in words. The editor pairs this with the
 * wall-clock time because a bare "Thu 23:15" five minutes before midnight
 * on a Thursday reads as *next* Thursday - the relative phrase is what
 * makes near occurrences unmistakable.
 */
export function relativeWhen(targetMs, nowMs) {
