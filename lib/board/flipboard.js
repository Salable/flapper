...
export class Flipboard {
  ...
  tint() {
    ...
    if (this._tintKey !== key) {
      ...
      this._runnerHead = null;
    }
    // A runner is a function of the clock, so it is rebuilt whenever its head
    // moves to the next cell - not every frame, and not never.
    if (spec?.runner) {
      const ring = perimeter(cols, rows).length;
      const head = runnerHead(ring, Number(spec.runner.periodMs), this.now());
      if (head !== this._runnerHead) {
        this._runnerHead = head;
        this._tintGrid = tintGrid(spec, cols, rows, this.now());
      }
      return this._tintGrid;
    }
  ...
  /**
   * Whether the wash itself is moving - drifting in hue, or running round the
   * edge. Either is motion with nothing in flight, which is the whole point of
   * a sign that is not dead.
   */
  isDrifting() {
    const spec = this.opts.tint ?? this.skin?.tint ?? null;
    return driftPeriod(spec) > 0 || Boolean(spec?.runner);
  }
   ...
   * a Marquee board would claim to be mid-flip for ever and liveness would call
   * it frozen the first time it missed a frame budget. Whether the *frame loop*
   * should keep running is a different question: see needsFrames().
   */
  ...
  setRegionPage(regionId, lines, { immediate = false, sweepBasis = 'region' } = {}) {
    ...
    if (immediate) {
       ...
       * track.mjs uses to resume, re-hold and blank, so Marquee only ever
       * moved if some unrelated event - a resize, a DPR change - happened to
       * repaint it. On a wall, never.
       */
