...
const FIDGET_DEFAULTS = Object.freeze({
   ...
   *
   * The difference between this and `flight` is the whole difference between
   * a coloured card and a coloured rectangle. Flight is a fill laid over the
   * finished tile every frame: it can tint the card (and on a dark design
   * that means grey) or cover it (and the hinge, the sheen and the falling
   * flap go with it). This bakes real cards in the colour instead - the same
   * paintCard, the same edge and sheen and grunge - so what flips past is a
   * card that happens to be pineapple. Painted blank: a card wearing a
   * colour has nothing to say.
   */
  cardWash: null,
  /**
