...
export const PACK_DEFAULTS = Object.freeze({
  ...
  // black edge 3% wide. Vignette and grunge push the flat procedural fill
  // toward a card that has actually sat on a wall - painted, not printed.
  card: Object.freeze({ fill: '#2b2b2b', edge: '#000000', radius: 0.05, inset: 0.03, sheen: 0.06, vignette: 0.22, grunge: 0.14 }),
  // A real vane's top edge catches the light just above the gap it hangs
  // over (`highlight`), and the pin it turns on reads as a pin rather than a
  // smudge - both were present in code and off/invisible by default. Band
  // height up from the 11% measured off Classic - true to the source, but
  // photographed close up a real gap reads as more than that, and 11% was
  // easy to mistake for a decorative line rather than a mechanical seam.
  hinge: Object.freeze({ fill: '#141414', highlight: 'rgba(255,255,255,0.16)', thickness: 0.15, pin: '#3c3c3e', pinWidth: 0.045, pinHeight: 0.1 }),
  glyph: Object.freeze({
    fill: '#efe9df',
    stroke: null,
    strokeWidth: 0.03,
  ...
  advanced: Object.freeze({
    ...
    fastStepMs: 55,
    landStepMs: 190,
    // A lower visual frame rate than the display's own - the canvas is
    // repainted at most this often, though the flap's actual step timing
    // above is untouched. A real board's mechanical clack never reads as a
    // smooth 60fps blur, and neither should this. 0 would mean uncapped.
    frameMs: 70,
    sweepMs: 300,
  ...
  /**
   * The space around and between the cards - the gaps, and the padding
   * outside the grid. Not a card property: it never flips, never shows a
   * glyph, and there is exactly one of it per board rather than one per
   * tile, so it lives at the top of the pack alongside tint and flight
   * rather than under `card`.
   */
  background: '#0a0a0b',
  /**
...
export function validatePack(input) {
  ...
  const pack = {
    ...
    states: {},
    art: { ...(input.art || {}) },
    background: input.background === undefined ? PACK_DEFAULTS.background : input.background,
  ...
  for (const key of ['card.fill', 'card.edge', 'hinge.fill', 'hinge.pin', 'glyph.fill', 'background']) {
  ...
  if (pack.tint !== null) {
    ...
    if (typeof tint !== 'object' || Array.isArray(tint)) {
      ...
      for (const key of Object.keys(tint)) {
        if (!['gradient', 'corners', 'mode', 'strength', 'drift'].includes(key)) {
      ...
      if (tint.corners !== undefined) {
        ...
        errors.push('tint needs a gradient or four corners');
