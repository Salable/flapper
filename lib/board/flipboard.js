...
export class Flipboard {
  ...
  isAnimating(regionId) {
    // A drifting wash is motion with nothing in flight, so the loop has to keep
    // going for it - which is the whole point of a sign that is not dead.
    if (regionId === undefined) return this.isDrifting() || this.tiles.some((tile) => tile.pending > 0);
