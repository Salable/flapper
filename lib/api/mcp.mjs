...
import { z } from 'zod';
import { FIDGETS } from '../board/fidgets.mjs';
import { ANIMATION_IDS } from '../board/animations.mjs';
import { eq } from 'drizzle-orm';
import pkg from '../../package.json' with { type: 'json' };
import * as boards from '../db/boards.mjs';
import { TEMPLATES } from '../board-types/templates.mjs';
import { THEMES, THEME_IDS } from '../board/themes.mjs';
import { CARD_SIZE_IDS } from '../board/geometry.mjs';

/** The fidgets a caller may name, for the enum below and its own message. */
const FIDGET_IDS = /** @type {[string, ...string[]]} */ (Object.keys(FIDGETS));

/** "classic (Charcoal tiles, bone glyphs - the original.), sorbet (...)" for the tool description. */
function themeSummary() {
  return Object.values(THEMES).map((theme) => `${theme.id} (${theme.description})`).join(', ');
}
...
connected, frozen whether it can actually animate (a background browser tab
cannot - ask the user to bring it to the front), and its lines field is what
the display was last told to show. Do not clear a board to
...
/** An animation instead of words - the other kind of content an item can
 * carry, never alongside text. */
const animationField = z
  .enum(ANIMATION_IDS)
  .describe('Show a designed animation instead of words. Mutually exclusive with text and rows.');
...
const TOOLS = [
  ...
  },
  {
    ...
    config: {
      title: 'Board theme',
      description:
        'The theme the board’s displays draw: the preset (`theme`), the board’s own overrides (`themePack`, null if none), and the resolved pack. Read this before changing a theme so you change only what you mean to.',
      inputSchema: z.object({}),
      annotations: { readOnlyHint: true },
    },
    call: () => ({ handler: getTheme, path: '/theme' }),
  },
  {
    name: 'get_status',
    config: {
      title: 'Board status',
      description:
        'What the glass shows: showing is the message on the glass in every state (the one playing, or the finished one still standing - held: true; phase is playing, holding, or blank); lines is the rows the display was last told to show (while animating is true the tiles are still turning toward them); plus the queue summary (server truth). boardReady: true means a display reported within the last few seconds; stale: true means none has, so lines may be old and nothing is showing new messages - tell the user to open the board URL on a screen. frozen: true means a display is connected but its browser tab is in the background, so it heartbeats but cannot animate - ask the user to bring the tab to the front or use the kiosk shell. The cheapest way to confirm a message landed.',
      inputSchema: z.object({}),
      annotations: { readOnlyHint: true },
    },
  ...
  },
  {
    ...
    config: {
      ...
      inputSchema: z.object({
        ...
        animation: animationField.optional(),
      }),
    },
  ...
  },
  {
    ...
    config: {
      ...
      description:
        'Change an item’s text, rows or animation, toggle loop, or change its schedule (clock boards).',
      inputSchema: z.object({
        itemId: z.string(),
        text: textField.optional(),
        rows: rowsField.optional(),
    ...
    config: {
      ...
      inputSchema: z.looseObject({
        ...
        theme: z
          .enum(THEME_IDS)
          .optional()
          .describe(`Theme preset: ${themeSummary()}. Applies to every display of the board.`),
        themePack: z
          .union([z.null(), z.looseObject({})])
          .optional()
          .describe(
            'The board’s own look on top of its preset: an object with any of card, hinge, glyph, motion, advanced (Hold, Travel speed, Landing, Frame gap, Sweep, Sweep shape, Full revolution - merged a level deep), states, art, fonts (replaced whole), plus background (the colour around and between the cards), tint (a wash across the grid), flight and flightStrength (colours a tile passes through while moving). The server stores only what differs from the preset; null resets to the preset. Fields, ranges and size limits: get_capabilities → themePack. Read get_theme first.',
          ),
        cardSize: z
          .union([z.null(), z.enum(CARD_SIZE_IDS)])
          .optional()
          .describe(
            'Cards across, biggest first: huge, large, medium, small, tiny. Rows follow from this and screen. null resets to the default (medium).',
          ),
        screen: z
          .union([z.null(), z.object({ w: z.number().positive(), h: z.number().positive() })])
          .optional()
          .describe(
            'The shape of the screen this board is designed for - any two positive numbers, any units (16 and 9, or a ticker\'s 300 and 20): only the ratio matters, since a board fills whatever window it is in. Turns cardSize into a row count. null resets to the default (16:9).',
          ),
        ambientMs: z
          .union([z.literal(0), z.number().min(5000).max(600000)])
          .optional()
          .describe(
            'Whether the board fidgets while it holds still: 0 is off, anything else is on. How often it happens is part of the fidget itself, so the number is only ever an on switch - pick which fidget with `fidget`.',
          ),
        fidget: z
          .union([z.null(), z.enum(FIDGET_IDS)])
          .optional()
          .describe(
            `Which fidget the board does while it holds still: ${FIDGET_IDS.join(', ')}. Each one carries its own pace, so this is the whole choice. null is the quiet one. Needs ambientMs above 0 to happen at all.`,
          ),
...
const USER_TOOLS = [
  ...
  },
  {
    ...
    config: {
      ...
      description:
        'Create a new board on your account. Returns its slug and URLs - not its API key, which is never emitted as a side effect; call get_board_key if a display or script genuinely needs it. Types: live (a played queue), scheduled (a clock plays items by schedule; takes timezone), shared (scheduled, for many screens). (Requires an OAuth connection.)',
      inputSchema: z.object({
        ...
        template: z
          .enum([...TEMPLATES.keys()])
          .optional()
          .describe('Start from a template: its type, preset config and a seeded queue; name/timezone/fallback here override it. Omit for a plain board of `type` - a live one still gets one placeholder slide so it never lands truly empty; a scheduled one has nothing to guess a schedule from and starts with none.'),
