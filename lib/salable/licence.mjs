...
export const REQUESTABLE = Object.freeze({
  ...
  interrupters: 'More interrupters per board',
...
/**
 * Three boards, live queue, public. Was one - Neal's call, 2026-09-02: "you
 * give everyone one board, and if they come and ask us for it we cut them a
 * cheap license". Dan's call, 2026-09-04, overriding that after being asked
 * to confirm it directly: three boards, three slides each, one interrupter
 * each - this is that decision, recorded rather than silently changed. This
 * constant is only the shape of the free plan and the outage fallback; the
 * free plan in Salable is what actually grants it, and changing the number
 * there changes it in production without a deploy.
 */
export const FREE_ALLOWANCE = Object.freeze({
  ...
  maxQueueItems: 3,
  // Dan's call, same day: one saved interrupter per board on free. The
  // flat MAX_INTERRUPTERS = 20 in handlers.mjs still applies on top -
  // same relationship as maxQueueItems has to queueCap's own range.
  maxInterrupters: 1,
});
...
function unlicensedAccount(source) {
  return Object.freeze({
    ...
    maxInterrupters: FREE_ALLOWANCE.maxInterrupters,
    source,
  });
}
...
export function allowanceFrom(values, source = 'salable') {
  ...
  return Object.freeze({
    ...
    // Same reasoning, same tie to the board-count tier.
    maxInterrupters: maxBoards > FREE_ALLOWANCE.maxBoards ? Infinity : FREE_ALLOWANCE.maxInterrupters,
    source,
  });
}
