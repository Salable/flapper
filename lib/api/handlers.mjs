...
/** Flat abuse guard, not an entitlement (tiers live in attic/ until Salable). */
...
/** Tell this board's displays that the queue or config moved. */
...
function clockTiming(board, payload, schedule) {
  const computedDurationMs = itemDurationMs(headlessController(manifest, board.config), payload);
...
export async function createBoard(request, { db, getSession }) {
  return handle(async () => {
    ...
    const config = migratedConfig(type, applyParams(type.createParams, body));
...
export async function postMessage(request, { broker, db, slug, getSession }) {
  return handle(async () => {
    ...
    const body = await readJsonBody(request, LIMITS.maxBodyBytes);
    const parsed = entryFromBody(body, 'api');
...
export async function getQueue(request, { db, slug, getSession }) {
  return handle(async () => {
    ...
    const config = type ? migratedConfig(type, board.config) : board.config;
    return json(200, {
      type: board.type,
...
export async function patchConfig(request, { broker, db, slug, getSession }) {
  return handle(async () => {
    ...
    const type = getBoardType(board.type);
    type?.validateConfig?.({ ...board.config, ...body });
