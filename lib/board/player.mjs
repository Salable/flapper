...
export class Player {
   ...
   * @param {object} [hooks] {onConfig(config, meta), onNote(text),
   *   onAnimation(name|null)} - meta carries what travels beside the config
   *   in a snapshot, today `themeRev`; onAnimation says when the item now on
   *   the glass is a designed motion rather than words, and when it isn't
   */
  ...
  showCard(text) {
    // Nothing is playing after this, so nothing may still be painting over
    // it. A paused board otherwise kept an animation running on top of
    // "BOARD PAUSED. SEE SETTINGS" - a wall that looks alive, saying
    // nothing, forever.
    this.hooks.onAnimation?.(null);
    try {
      this.controller.clear();
  ...
  playItem(item) {
    ...
    try {
      ...
      /*
       * An animation is content with no words: the item enqueued above is a
       * blank page held for its own dwell, which playback already knows how
       * to do, and this says who paints over it. Announced rather than run
       * here because lib/ owns no timers and no canvas - the display's
       * animator does (components/flapper/animator.ts).
       */
      this.hooks.onAnimation?.(options.animation ?? null);
