...
import { MAX_COLS, MAX_ROWS, CARD_SIZE_IDS } from '../board/geometry.mjs';
...
export function validateConfigPatch(body, current = {}) {
  ...
  for (const [key, max] of [['cols', MAX_COLS], ['rows', MAX_ROWS]]) {
    if (body[key] === undefined) continue;
    // `null` explicitly, as footerRows does: Number(null) is 0 and would read
    // as a board with no cards.
    const value = body[key] === null ? NaN : Number(body[key]);
    if (!Number.isInteger(value) || value < 1 || value > max) {
      reject(`${key} must be a whole number between 1 and ${max}`, 422);
    }
  }
