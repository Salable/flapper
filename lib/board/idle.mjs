...
export const FIDGET_STYLES = Object.freeze({
  ...
  'pina-colada': Object.freeze({
    ...
    sweepEvery: 0,
    restOdds: 2,
    // Slower than rainbow and two cards at a time - it is a long drink.
    stepDistance: 3,
    stepMs: 140,
    flickerCount: 2,
    holdMs: 320,
    returnStepMs: 140,
    /*
     * A short list, deliberately, where snake and sherbet ramp across the
     * whole ring - and it is worth saying why, because the two look like an
     * inconsistency to fix and are not.
     *
     * flightColour indexes by the tile's ring state, so a five-long list
     * cycles every five steps. A card travelling three steps therefore sees
     * three *different* stops of the drink. Ramped across forty-two instead,
     * those same three steps would be three neighbouring shades of one
     * colour - smoother, and completely the wrong effect. Ramp when the
     * journey is long or spread across many cards; keep the list short when
     * a single card makes a brief trip.
     */
    flight: Object.freeze(['#f2d16b', '#f7f0e0', '#7fbf5f', '#f7f0e0', '#e8c05a']),
    flightStrength: 0.85,
  }),
  /**
