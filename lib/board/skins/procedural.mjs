...
export class ProceduralSkin {
  ...
  constructor(pack, { arts = new Map(), createCanvas } = {}) {
    ...
    /** The space around and between the cards - see flipboard.js's draw(). */
    this.background = pack.background ?? null;
  }

  /** (Re)build the cards when the tile size changes. */
  /**
   * Bake an alternate set of cards, faces painted from `palette`.
   *
   * Real cards, not a colour laid over the top of one. They go through the
   * same paintCard as everything else, so they keep the edge, the sheen
   * step at the hinge, the grunge and the vignette - a card that happens to
   * be pineapple rather than a pineapple rectangle. Painted blank, because
   * a card wearing a colour has nothing to say.
   *
   * Returned rather than stored: a skin is shared property (one per tab,
   * used by the wordmark and every board at once), so a board that wants a
   * wash keeps its own set and never touches this one's.
   *
   * @param {number} size tile size in px, as prepare() takes
   * @param {string[]} palette one colour per ring step, cycled if shorter
   * @param {boolean} [glyphs] keep each state's own character on its card.
   *   Off (the default) paints them blank, which is right when the colour is
   *   the whole gesture and a letter would only be noise. On is for anything
   *   *spelled* out of the ring - a creature drawn from `(` and `)` needs
   *   its glyphs or there is nothing left of it but a coloured square.
   */
  bakeWash(size, palette, glyphs = false) {
    if (!Array.isArray(palette) || palette.length === 0) return null;
    return this.cycle.map((state, index) => {
      const canvas = this.createCanvas(size);
      const ctx = canvas.getContext('2d');
      const style = resolveStateStyle(this.pack, state.char);
      const washed = { ...style, card: { ...style.card, fill: palette[index % palette.length] } };
      const char = glyphs ? state.char : ' ';
      const art = glyphs && washed.art ? this.arts.get(washed.art) || null : null;
      paintCard(ctx, size, char, washed, art);
      return canvas;
    });
  }
  ...
  /**
   * @param {HTMLCanvasElement[]} [cards] an alternate card set (see
   *   `bakeWash`) - the caller's, never this skin's, and only for the tiles
   *   it wants wearing it.
   */
  drawTile(ctx, state, progress, x, y, size, cards) {
    if (size !== this.cardSize) this.prepare(size);
    const { motion, hinge, card } = this.pack;
    const set = cards ?? this.cards;
    const cur = set[state];
    const next = set[(state + 1) % set.length];
    const h = size / 2;
    ...
    const sinTheta = Math.sin(theta); // read four times over below; computed once

    ctx.save();
    ctx.translate(x, y);

    // The static halves - already correctly shaped (paintCard baked its own
    // edge and face, rounded corners included), so no clip is needed here.
    ...
    if (inFlight) {
      ...
      //
      // Clipped to the inset face (the same rounded rect card.fill itself
      // is drawn into), not the full tile: a plain rect spanning the whole
      // tile ignores both the rounding and the edge, and washed a lighter
      // rectangle straight over what should have stayed a flat black bezel.
      // Scoped tightly around just this fillRect - not the flap drawn
      // below, which needs its own, different clip; see there.
      if (ctx.clip) {
        ctx.save();
        roundedRect(ctx, inset, inset, size - inset * 2, size - inset * 2, Math.max(0, size * card.radius - inset));
        ctx.clip();
      }
      ctx.fillStyle = `rgba(0,0,0,${(motion.shading * 0.6 * sinTheta).toFixed(3)})`;
      ctx.fillRect(0, 0, size, size);
      if (ctx.clip) ctx.restore();
      ...
      const lit = motion.highlight * sinTheta;
      ...
      if (k >= 0) {
        ...
        ctx.translate(0, -h);
        // cur's own edge and face are already correctly shaped (paintCard
        // baked them), so the drawImage itself is never clipped - only
        // shade()'s highlight overlay below is, and only to the three
        // edges this half actually borders (left, right, top; y=h here is
        // the hinge seam, not the card's own border, so it stays unclipped
        // there, matching the highlight reaching all the way to the fold).
        ctx.drawImage(cur, 0, 0, size, h, 0, 0, size, h);
        if (ctx.clip) {
          ctx.save();
          ctx.beginPath();
          ctx.rect(inset, inset, size - inset * 2, h - inset);
          ctx.clip();
          shade(ctx, size, h, lit);
          ctx.restore();
        } else {
          shade(ctx, size, h, lit);
        }
        ...
        // Same reasoning, mirrored: y=0 here is the hinge seam (unclipped),
        // y=h is the card's true bottom edge (inset).
        if (ctx.clip) {
          ctx.save();
          ctx.beginPath();
          ctx.rect(inset, 0, size - inset * 2, h - inset);
          ctx.clip();
          shade(ctx, size, h, lit);
          ctx.restore();
        } else {
    ...
    // gap with one soft gradient peak read as a glow sitting on the card
    // rather than a seam cut into it - real depth has a flat bottom, not
    // just a point. Flat fill, no feather: canvas already antialiases the
    // rect's own edge at the sub-pixel level, so a manual gradient feather
    // on top of that was solving a problem that didn't exist - it just
    // made the edge softer than it needed to be.
    const band = size * hinge.thickness;
    ctx.fillStyle = hinge.fill;
    ctx.fillRect(0, h - band / 2, size, band);
    if (hinge.highlight) {
      // A bevel: the lower flap's top edge catching the light just below
      // centre, inside the band rather than floating below it in plain
      // card colour - which is what a fixed offset measured against the
      // old, much thinner band left once the band itself grew to 0.15 and
      // then lost its feather entirely, becoming one flat fill the width
      // of `band`. Sits inside that fill (not at its true lower edge,
      // h + band/2) because that read better once tried - a highlight
      // right at the band's edge looked like a second, competing line
      // rather than one bevel on the seam already there.
    ...
    if (inFlight && ctx.createLinearGradient) {
      ...
      const bleed = size * 0.07 * sinTheta;
      const g = ctx.createLinearGradient(0, size, 0, size + bleed);
      g.addColorStop(0, `rgba(0,0,0,${(motion.shadow * 0.6 * sinTheta).toFixed(3)})`);
      g.addColorStop(1, 'rgba(0,0,0,0)');
      ctx.fillStyle = g;
...
/**
 * A little sheen near a flap's fold, as it catches more light passing
 * flat-on. Used to darken the flap itself too, before fcf5bb3 reversed
 * the lighting model so the flap brightens rather than shades as it
 * turns - both call sites always pass 0 for that now, so the parameter
 * (and the branch that read it) is gone rather than kept as dead weight.
 */
function shade(ctx, w, h, sheen) {
