...
/**
 * A held row's standing page is over the moment something new arrives - drop
 * it before anything counts the queue toward its cap, so the slot it still
 * occupies is not charged against a fresh insert that would free it anyway.
 * Board row must be locked. Returns the board, patched if a row was dropped.
 */
async function dropStaleHold(tx, board) {
  if (playbackOf(board) === 'live' && board.currentState === 'holding' && board.currentItemId) {
    ...
    return { ...board, currentItemId: null, currentState: 'idle' };
  }
  return board;
}
 ...
 */
/**
 * `enforceTypeCap: false` (setNow alone) skips the type's own cap/roll -
 * the MAX_ITEMS backstop below still always applies. A `now` insert is
 * about to demote whatever is current to an ordinary queued row, which is
 * exactly the row `isPending` would let this same call roll if it weren't
 * still, technically, `currentItemId` for the next few lines - so on a
 * cap-1 board (a "sign"), every `now` found itself with one protected
 * item, nothing eligible to roll, and rejected outright. A board that
 * cannot be interrupted defeats the one thing `now` is for.
 */
async function makeRoom(tx, board, { enforceTypeCap = true } = {}) {
...
/**
 * Set the head when nothing is playing: promotes the lowest item. A stale
 * held row is not this function's business - callers that might be looking
 * at one drop it with `dropStaleHold` first, so by the time it runs
 * `currentState` is never 'holding'. Board row must be locked. Returns the
 * patch applied to the board, or null.
 */
...
export async function appendItem(db, boardId, entry) {
  return db.transaction(async (tx) => {
    let board = await lockBoard(tx, boardId);
    board = await dropStaleHold(tx, board);
...
export async function setNow(db, boardId, entry) {
  return db.transaction(async (tx) => {
    ...
    // and (see dropStaleHold) must not be charged against the cap below.
    board = await dropStaleHold(tx, board);
    await makeRoom(tx, board, { enforceTypeCap: false });
...
/**
 * Remove every queued instance of one fired interrupter, not just whichever
 * one happens to be on the glass. A saved interrupter can be fired again
 * while it is already live - nothing stops it, and Fire stays clickable
 * even once muted - which queues a second copy behind the first instead of
 * replacing it. Dismissing only the current item in that state just
 * promotes an identical clone into its place: the glass still reads
 * exactly the same, which looks like the button did nothing. `label` is
 * matched case-insensitively, the same identity `fireInterrupter` itself
 * assigns and checks rank against.
 */
export async function removeByLabel(db, boardId, label) {
  ...
  return db.transaction(async (tx) => {
    ...
    const matches = items.filter(
      (item) =>
        item.payload?.options?.interrupt === true &&
        String(item.payload?.options?.label ?? '').toLowerCase() === needle,
    );
    if (matches.length === 0) return { removed: 0 };
    await tx.delete(queueItems).where(
      inArray(queueItems.id, matches.map((item) => item.id)),
    );
    await touchQueue(tx, board.queueId);
    if (playbackOf(board) === 'live' && matches.some((item) => item.id === board.currentItemId)) {
      await promoteIfNotPlaying(tx, { ...board, currentState: 'idle', currentItemId: null });
    }
...
/**
 * Live boards: the same sweep, but a live board's `currentItemId` is a real
 * playback head, not just a row that happens to exist - an expired item
 * that was the one showing needs the board actually moved on, not just its
 * row gone out from under a still-"playing" board. Reuses `advance()`'s own
 * "row vanished under us" recovery: force the state to 'idle' so
 * `promoteIfNotPlaying`'s playing-guard does not block picking a new head.
 */
export async function sweepExpiredLive(db, boardId, nowMs) {
  return db.transaction(async (tx) => {
    const board = await lockBoard(tx, boardId);
    const gone = await tx
      .delete(queueItems)
    ...
    if (gone.some((row) => row.id === board.currentItemId)) {
      await promoteIfNotPlaying(tx, { ...board, currentState: 'idle' });
    }
    await touchQueue(tx, board.queueId);
    return gone.length;
  });
