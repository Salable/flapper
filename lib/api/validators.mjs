...
import { validateSchedule, isTimezone, periodOf } from '../board/schedule.mjs';
import { ANIMATION_IDS } from '../board/animations.mjs';
...
/**
 * `animation`: the item is a designed motion rather than words.
 *
 * Content, not a trigger and not a source - an animation has no words and
 * nothing feeds it (see TODO.md, *An interrupter's trigger, not its
 * source*). It rides on the options rather than replacing the payload so
 * that everything already true of an item - dwell, priority, interrupt,
 * label - stays true of this one, and a build that has never heard of the
 * name still has a well-formed item to hold.
 */
function animationOption(body) {
  if (body.animation === undefined) return undefined;
  if (body.animation === null) return undefined;
  if (typeof body.animation !== 'string' || !ANIMATION_IDS.includes(body.animation)) {
    reject(`animation must be one of ${ANIMATION_IDS.join(', ')}`, 422);
  }
...
export function textOptions(body, limits = LIMITS) {
  ...
  if (rows !== undefined) {
    ...
    // `null` is "no animation", which is what rows already are - only a
    // named one is a contradiction. Matching the text branch, where null is
    // likewise a no-op rather than a refusal.
    if (body.animation !== undefined && body.animation !== null) {
      reject('animation does not apply when rows is given; an animation has no cells to take literally', 422);
    }
    return { text, options };
  }
  ...
  const animated = animationOption(body);
  if (animated !== undefined) {
    if (text.trim() !== '') {
      reject('animation and text are the two kinds of content, not a pair - send one', 422);
    }
    options.animation = animated;
  }
  return { text, options };
}
...
export function validateConfigPatch(body, current = {}) {
  ...
  /*
   * Saved interrupters are config, but they are not *this* door's business.
   *
   * `config.interrupters` has its own routes (POST /interrupters and
   * friends), which is where the licence ceiling, the flat MAX_INTERRUPTERS
   * backstop, the name rules and the content validation all live. A PATCH
   * landing the whole array here walked past every one of them - and once a
   * live board's queue read began firing scheduled interrupters, that was no
   * longer only a tidiness problem: an array of presets written straight
   * into config is work that any reader, including an anonymous one on a
   * public board, then makes the server do.
   *
   * Refused rather than merged, so nobody can half-write the list through a
   * door that never checked it.
   */
  if (body.interrupters !== undefined) {
    reject('interrupters are not set through /config - use POST /interrupters, which checks names, content and your licence', 422);
  }
  let themePack;
...
export function validateInterrupterPreset(body, limits = LIMITS) {
  ...
  if (rows !== undefined) {
    ...
    // Blank is allowed, and deliberately: `+ Interrupt` creates the row on
    // the click the way `+ Slide` does, and there is nothing to type yet at
    // that moment. A blank interrupter shows a blank board when it fires,
    // which is exactly what a blank slide does in the rotation.
    if (typeof body.text !== 'string') {
      reject('text must be a string (or send rows instead)', 422);
    }
  ...
  // An interrupter can be an animation instead of words, the same either-or
  // an item has - Dan, 15 Sep 2026: animations are a what, not a when.
  if (body.animation !== undefined && body.animation !== null) {
    if (!ANIMATION_IDS.includes(body.animation)) {
      reject(`animation must be one of ${ANIMATION_IDS.join(', ')}`, 422);
    }
    if ((preset.text ?? '').trim() !== '' || preset.rows !== undefined) {
      reject('animation and text are the two kinds of content, not a pair - send one', 422);
    }
    preset.animation = body.animation;
  }

  // An interrupter with a schedule is shown by the clock rather than fired:
  // its window opens at each occurrence and closes durationMs later, which
  // is why a duration stops being optional here. "Until dismissed" is a
  // switch somebody throws, and nobody is there to throw it at 5pm.
  if (body.schedule !== undefined && body.schedule !== null) {
    /*
     * A scheduled *board's* item takes `schedule.durationMs` - that is how
     * long its slot is (see lib/board/schedule.mjs). An interrupter's window
     * is its own `durationMs`, one level up, and the one inside the spec is
     * read by nothing here. Said rather than dropped: it is a field that
     * exists, in a place where it does nothing, which is the worst kind.
     */
    if (body.schedule.durationMs !== undefined) {
      reject('an interrupter\'s window is durationMs beside the schedule, not inside it', 422);
    }
    preset.schedule = validateSchedule(body.schedule);
    if (preset.durationMs === undefined) {
      reject('a scheduled interrupter needs a durationMs - its window has to close on its own', 422);
    }
    /*
     * A window has to close before the next one opens.
     *
     * Otherwise every occurrence fires while the last is still on the glass
     * and the copies stack: "every 5 minutes for an hour" is not a longer
     * announcement, it is twelve of them queued behind each other, and the
     * board never returns to its rotation. Refused here rather than handled
     * at fire time, because the spec is the thing that is wrong.
     */
    const period = periodOf(preset.schedule);
    if (period !== null && preset.durationMs > period) {
      reject(
        `durationMs ${preset.durationMs} is longer than the gap between occurrences (${period}ms) - each one would still be showing when the next fires`,
        422,
      );
    }
    // The zone the wall-clock kinds are read in. Same field name and same
    // validity test the scheduled board type uses for its own items.
    if (body.timezone !== undefined) {
      if (typeof body.timezone !== 'string' || !isTimezone(body.timezone)) {
        reject('timezone must be an IANA zone name like "Europe/London"', 422);
      }
      preset.timezone = body.timezone;
    }
  } else if (body.timezone !== undefined) {
    reject('timezone only applies to a scheduled interrupter', 422);
  }
