...
const BASE_ASPECT = DEFAULT_SCREEN.w / DEFAULT_SCREEN.h;

/**
 * Cards across, for a size name, on a screen of a given aspect ratio.
 *
 * CARD_SIZES calibrates each name against the default 16:9 screen - "medium"
 * is 20 across a 16:9 wall, 220 cards total once rows go square to it
 * (`totalAtBase` below). Holding cols at that fixed number for every shape
 * was wrong: rotate the same physical screen from 16:9 to portrait 9:16 and
 * cols stayed 20 while rows blew out to 36 to stay square, well over
 * tripling the card count for a screen that is the same area, just turned
 * on its side. Scaling cols by the square root of the aspect ratio instead
 * keeps `cols * rows` (see `gridFor`) close to that 220 across every shape -
 * exactly equal to it at 16:9 itself, since that is what CARD_SIZES was
 * calibrated against.
 */
function colsForCardSize(size, aspect) {
  const base = CARD_SIZES[String(size)] ?? CARD_SIZES[DEFAULT_CARD_SIZE];
  const totalAtBase = base * clampRows(base / BASE_ASPECT);
  const a = Number.isFinite(aspect) && aspect > 0 ? aspect : BASE_ASPECT;
  return Math.round(Math.sqrt(totalAtBase * a));
}
 ...
 * the size and the shape together, scaled to hold roughly the same total
 * regardless of orientation (see `colsForCardSize`); cards down is whatever
 * keeps them square on that shape. A 15:1 ticker and a 16:9 wall both work,
 * because the shape is any two numbers.
 *
...
export function gridFor(size = DEFAULT_CARD_SIZE, screen = DEFAULT_SCREEN, cardAspect = 1) {
  const w = Number(screen?.w);
  const h = Number(screen?.h);
  const aspect = Number.isFinite(w) && w > 0 && Number.isFinite(h) && h > 0 ? w / h : BASE_ASPECT;
  const cols = Math.max(1, Math.min(MAX_COLS, colsForCardSize(size, aspect)));
