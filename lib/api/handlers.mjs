...
import { getUserTier, boardLimitFor, can } from '../db/entitlements.mjs';
import { displayTokenValid } from './display-token.mjs';
import { recompileQueue } from './timeline.mjs';
import { nextBoundaryMs } from '../board/schedule.mjs';
...
/**
 * Tell every display of this board's queue that something moved. A shared
 * queue nudges every attached board's stream; each display then refetches
 * against its own board (heads differ per board in live mode).
 */
async function nudge(broker, db, boardId, method = 'sync') {
  const snapshot = await queue.listQueue(db, boardId);
  const attached = snapshot
    ? await queuesDb.boardsOfQueue(db, snapshot.queueId)
    : [{ id: boardId }];
  for (const sibling of attached) {
    const own = sibling.id === boardId ? snapshot : await queue.listQueue(db, sibling.id);
    await broker.appendCommand(sibling.id, {
      method,
      params: own
        ? {
            currentItemId: own.currentItemId,
            currentState: own.currentState,
            epoch: own.epoch,
            queueUpdatedAt: new Date(own.queueUpdatedAt).getTime(),
          }
        : {},
      source: 'api',
    });
    await broker.touch(sibling.id);
  }
}

/** Recompile the timed cycle (durations use the primary board's config). */
async function recompileIfTimed(db, board) {
  const snapshot = await queue.listQueue(db, board.id);
  if (snapshot?.mode !== 'timed') return;
  const [primary] = await queuesDb.boardsOfQueue(db, snapshot.queueId);
  await recompileQueue(db, primary ?? board);
}

/**
 * Dormancy: what an under-entitled configuration means for this board
 * (RFC 0002 decision 4 - display is configurable, data untouched).
 */
async function dormancyOf(db, board, snapshot) {
  const tier = await getUserTier(db, board.ownerId);
  if (snapshot.mode === 'timed' && !can(tier, 'scheduling')) {
    return { dormant: true, reason: 'time-based queues need the Plus offering' };
  }
  const attached = await queuesDb.boardsOfQueue(db, snapshot.queueId);
  if (attached.length > 1 && !can(tier, 'sharedQueues') && attached[0].id !== board.id) {
    return { dormant: true, reason: 'shared queues need the Plus offering' };
  }
...
export async function postMessage(request, { broker, db, slug, getSession }) {
  return handle(async () => {
    ...
    const snapshot = await queue.listQueue(db, board.id);
    if (snapshot.mode === 'timed' && !entry.loop) {
      // Timed one-shots are spliced: 'now' starts immediately, everything
      // else at the end of the current slot, played once, then swept.
      const nowMs = Date.now();
      entry.playAtMs =
        priority === 'now'
          ? nowMs
          : nextBoundaryMs(
              { items: snapshot.items, cycleAnchorMs: snapshot.cycleAnchorMs, cycleMs: snapshot.cycleMs },
              nowMs,
            );
    }
    const { item } =
      priority === 'now' && snapshot.mode === 'live'
        ? await queue.setNow(db, board.id, entry)
        : priority === 'next' && snapshot.mode === 'live'
          ? await queue.insertAfterCurrent(db, board.id, entry)
          : await queue.appendItem(db, board.id, entry);
    await recompileIfTimed(db, board);
...
export async function getQueue(request, { db, slug, getSession }) {
  return handle(async () => {
    ...
    if (snapshot.mode === 'timed') {
      // One-shots whose moment has fully passed are done; sweep lazily.
      if ((await queue.sweepExpiredOneShots(db, snapshot.queueId, Date.now())) > 0) {
        snapshot = await queue.listQueue(db, board.id);
      }
    }
    const dormancy = await dormancyOf(db, board, snapshot);
    return json(200, {
      mode: snapshot.mode,
      currentItemId: snapshot.currentItemId,
      currentState: snapshot.currentState,
      ...
      ...dormancy,
      dormancyDisplay: snapshot.dormancyDisplay,
...
export async function advanceQueue(request, { broker, db, slug, getSession }) {
  return handle(async () => {
    ...
    const snapshotMode = (await queue.listQueue(db, board.id)).mode;
    if (snapshotMode !== 'live') {
      return json(200, { ok: true, advanced: false, mode: snapshotMode });
    }
...
export async function patchConfig(request, { broker, db, slug, getSession }) {
  return handle(async () => {
    ...
    // Stored for capabilities/preview; connected displays refetch on the
    // nudge. Timed slots are sized from config, so the cycle recompiles.
    await recompileIfTimed(db, { ...board, config });
    await nudge(broker, db, board.id);
    return json(200, { ok: true, config });
  });
}

/* ---- queue mode, sharing, and the offering ---- */

/** POST {mode?, dormancyDisplay?}: the RFC 0002 toggle. Timed is Plus. */
export async function queueMode(request, { broker, db, slug, getSession }) {
  return handle(async () => {
    const board = await requireBoardBySlug(db, slug);
    ...
    if (body.mode === 'timed') {
      const tier = await getUserTier(db, session.user.id);
      if (!can(tier, 'scheduling')) {
        reject('time-based queues are part of the Plus offering; switch your offering first', 403);
      }
    }
    const updated = await queuesDb.setMode(db, board.queueId, {
      mode: body.mode,
      dormancyDisplay: body.dormancyDisplay,
    });
    if (updated.mode === 'timed') await recompileIfTimed(db, board);
    await nudge(broker, db, board.id);
    return json(200, {
      ok: true,
      mode: updated.mode,
      dormancyDisplay: updated.dormancyDisplay,
    });
  });
}

/** POST {board: otherSlug}: attach another of your boards to this queue. */
export async function queueAttach(request, { broker, db, slug, getSession }) {
  return handle(async () => {
    const board = await requireBoardBySlug(db, slug);
    const session = await requireOwner(board, getSession);
    const tier = await getUserTier(db, session.user.id);
    if (!can(tier, 'sharedQueues')) {
      reject('sharing a queue across boards is part of the Plus offering', 403);
    }
    const body = await readJsonBody(request, LIMITS.maxBodyBytes);
    if (typeof body.board !== 'string') reject('board must be a board slug', 422);
    const other = await requireBoardBySlug(db, body.board);
    if (other.ownerId !== board.ownerId) reject('you can only attach your own boards', 403);
    await queuesDb.attachBoard(db, board.queueId, other.id);
    await recompileIfTimed(db, board);
    await nudge(broker, db, board.id);
    return json(200, { ok: true, attached: other.slug });
  });
}

/** POST: detach THIS board from a shared queue onto a fresh empty one. */
export async function queueDetach(request, { broker, db, slug, getSession }) {
  return handle(async () => {
    const board = await requireBoardBySlug(db, slug);
    await requireOwner(board, getSession);
    await queuesDb.detachBoard(db, board.id);
    // The departing board gets its own nudge; the old queue's remaining
    // boards learn their sibling left (harmless refetch).
    await nudge(broker, db, board.id);
    return json(200, { ok: true });
  });
}
...
/**
 * POST {tier}: switch the account's offering. Free both ways while
 * monetization does not exist - the data model treats it as an entitlement
 * flip, which is the whole point: downgrade pauses, never destroys.
 */
export async function accountTier(request, { broker, db, getSession }) {
  return handle(async () => {
    const session = await getSession?.();
    if (!session) reject('sign in first', 401);
    const body = await readJsonBody(request, LIMITS.maxBodyBytes);
    if (!['standard', 'plus'].includes(body.tier)) {
      reject('tier must be standard or plus', 422);
    }
    const { user, queues } = await import('../db/schema.mjs');
    const { eq } = await import('drizzle-orm');
    await db
      .update(user)
      .set({ tier: body.tier, updatedAt: new Date() })
      .where(eq(user.id, session.user.id));
    // Dormancy is an entitlement effect: move every queue's watermark and
    // nudge, so displays refetch now instead of at a coincidental sync.
    await db
      .update(queues)
      .set({ updatedAt: new Date() })
      .where(eq(queues.ownerId, session.user.id));
    for (const board of await boards.listByOwner(db, session.user.id)) {
      await nudge(broker, db, board.id);
    }
    return json(200, { ok: true, tier: body.tier });
  });
}
