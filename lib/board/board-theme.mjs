...
import { sha256Hex } from '../broker/tokens.mjs';
...
const SECTIONS = Object.freeze(['card', 'hinge', 'glyph', 'motion', 'advanced']);
const PACK_KEYS = new Set([...SECTIONS, 'states', 'art', 'fonts', 'background', 'tint', 'flight', 'flightStrength']);
...
export function mergePack(preset, sparse) {
  ...
  merged.background = sparse?.background !== undefined ? sparse.background : (preset.background ?? '#0a0a0b');
  merged.tint = sparse?.tint !== undefined ? sparse.tint : (preset.tint ?? null);
  merged.flight = sparse?.flight !== undefined ? sparse.flight : (preset.flight ?? null);
  merged.flightStrength =
    sparse?.flightStrength !== undefined ? sparse.flightStrength : (preset.flightStrength ?? 1);
  return merged;
}
...
export function sparsify(full, preset) {
  ...
  if (!same(full.background ?? '#0a0a0b', preset.background ?? '#0a0a0b')) {
    out.background = full.background ?? '#0a0a0b';
  }
  if (!same(full.tint ?? null, preset.tint ?? null)) out.tint = full.tint ?? null;
  if (!same(full.flight ?? null, preset.flight ?? null)) out.flight = full.flight ?? null;
  if (!same(full.flightStrength ?? 1, preset.flightStrength ?? 1)) out.flightStrength = full.flightStrength ?? 1;
