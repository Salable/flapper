 ...
 */
export function tintGrid(tint, cols, rows, now = 0) {
  if (!tint || typeof tint !== 'object') return null;
  if (tint.runner) return runnerGrid(tint.runner, cols, rows, now);
...
/**
 * The cells around the edge of a grid, clockwise from the top-left.
 *
 * A one-row or one-column board is all edge, and a 2x2 has no inside, so the
 * general case has to not count a corner twice.
 */
export function perimeter(cols, rows) {
  const across = Math.floor(Number(cols));
  const down = Math.floor(Number(rows));
  if (!Number.isFinite(across) || !Number.isFinite(down) || across < 1 || down < 1) return [];
  if (down === 1) return Array.from({ length: across }, (_, col) => col);
  if (across === 1) return Array.from({ length: down }, (_, row) => row);

  const cells = [];
  for (let col = 0; col < across; col += 1) cells.push(col);
  for (let row = 1; row < down; row += 1) cells.push(row * across + across - 1);
  for (let col = across - 2; col >= 0; col -= 1) cells.push((down - 1) * across + col);
  for (let row = down - 2; row >= 1; row -= 1) cells.push(row * across);
  return cells;
}

/**
 * A colour running round the edge of the board.
 *
 * The oldest trick a sign has: a light that chases the border, so a board
 * saying one thing forever is still alive. Sparse - every cell that is not
 * under the runner is left alone, which the renderer already understands,
 * because a grid entry of nothing means no tint on that tile.
 *
 * @param {{colour?: string, length?: number, periodMs?: number}} runner
 * @param {number} now milliseconds; the head's position is a function of it
 */
export function runnerGrid(runner, cols, rows, now) {
  const rgb = parseHex(runner?.colour);
  const ring = perimeter(cols, rows);
  const period = Number(runner?.periodMs);
  if (!rgb || ring.length === 0 || !Number.isFinite(period) || period < 1000) return null;

  const length = Math.max(1, Math.min(ring.length, Math.floor(Number(runner?.length) || 1)));
  const head = runnerHead(ring.length, period, now);
  const grid = new Array(Math.floor(cols) * Math.floor(rows)).fill(null);
  for (let i = 0; i < length; i += 1) {
    // The tail fades, so the thing reads as travelling rather than as a block
    // of colour teleporting round the edge.
    const fade = length === 1 ? 1 : 1 - i / length;
    const at = ring[(((head - i) % ring.length) + ring.length) % ring.length];
    grid[at] = {
      r: Math.round(rgb[0] * fade),
      g: Math.round(rgb[1] * fade),
      b: Math.round(rgb[2] * fade),
    };
  }
  return grid;
}

/** Which cell of the perimeter the runner's head is on at `now`. */
export function runnerHead(ringLength, period, now) {
  if (!ringLength || !period) return 0;
  return Math.floor((((Number(now) || 0) % period) / period) * ringLength) % ringLength;
}
