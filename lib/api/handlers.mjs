...
export async function createBoard(request, { broker, db, getSession }) {
  return handle(async () => {
    ...
    if (body.designId !== undefined) {
      ...
      // A design and a board's override are held to different limits - a design
      // may be 256 KB, a board's pack 64 KB with eight arts - so a design that
      // is perfectly legal can be too big to wear. Checked here, because the
      // alternative is a 201 followed by every display quietly falling back to
      // the preset with nothing said anywhere.
      const fits = checkThemePackLimits(design.pack);
      if (!fits.ok) {
        reject(
...
function designPack(value) {
  if (value === undefined || value === null || typeof value !== 'object' || Array.isArray(value)) {
    // GET /api/designs returns the shipped presets alongside your designs -
    // there is no separate presets route, and pointing an agent at one it
    // cannot reach is worse than saying nothing.
    reject('pack must be a theme pack object; GET /api/designs lists the shipped ones to copy', 422);
  }
...
/** Make one. `basedOn` is a note about where it started, never resolved. */
export async function createDesignHandler(request, { db, getSession }) {
  return handle(async () => {
    ...
    // rather than a pack somebody had to fetch and paste back.
    const from = body.from === undefined ? null : String(body.from);
    if (from !== null && !isTheme(from)) {
      reject(`unknown design "${from}"; this build ships ${THEME_IDS.join(', ')}`, 422);
    }
    const pack = body.pack === undefined && from !== null ? resolveTheme(from) : designPack(body.pack);
