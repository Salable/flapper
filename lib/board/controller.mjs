...
import { MOTION_DEFAULTS } from './timing.mjs';
import { THEME_IDS } from './themes.mjs';
import { themeCapabilities } from './board-theme.mjs';
import { RANGES } from './theme-pack.mjs';
import { MAIN, FOOTER } from './regions.mjs';
import { Track, TRACK_DEFAULTS, PRIORITIES, MAX_DWELL_MS } from './track.mjs';

const CONTROLLER_DEFAULTS = Object.freeze({ ...TRACK_DEFAULTS });
...
export class Controller {
  ...
  constructor(board, options = {}) {
    ...
    this.onChange = null;
    /** Called when a message finishes all its pages and its final dwell. */
    this.onMessageDone = null;
  ...
  syncTracks() {
    for (const region of this.board.regions) {
      ...
      track.onMessageDone = (message) => this.onMessageDone?.(message, region.id);
  ...
  capabilities() {
    return {
      ...
      themes: [...THEME_IDS],
      // What a board may override per theme, and how much: see PATCH /config themePack.
      themePack: themeCapabilities(RANGES),
      priority: [...PRIORITIES],
  ...
  configure(patch = {}) {
    ...
    // and set its hold.
    if ('dwellMs' in patch) {
      const dwell = clampNumber(patch.dwellMs, 0, MAX_DWELL_MS);
  ...
  configureBands(regions) {
    ...
    for (const [id, band] of Object.entries(regions)) {
      ...
      if (band.dwellMs === null) delete entry.dwellMs;
        ...
        const dwell = clampNumber(band.dwellMs, 0, MAX_DWELL_MS);
