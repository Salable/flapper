...
/** The 42-state ring every board runs today, as the sprite manifest names it. */
...
export const PACK_DEFAULTS = Object.freeze({
  // Measured from the Classic art at 256 px: face #2b2b2b, a black edge 3%
  // wide, a soft hinge band ~11% tall, and pins too small to see.
...
export function validatePack(input) {
  ...
  for (const [key, value] of Object.entries(pack.art)) {
    ...
    if (typeof value !== 'string' || !/^(data:image\/|\/|https:\/\/)/.test(value)) {
      errors.push(`art.${key} must be an image URL or data URI`);
    }
  }
  pack.fonts.forEach((font, i) => {
    ...
    } else if (!/^(data:font\/|data:application\/|\/|https:\/\/)/.test(font.src)) {
      errors.push(`fonts[${i}].src must be a font URL or data URI`);
